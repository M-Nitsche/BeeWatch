...
def run_blob_det_correct_centroid_tracker(
        opt,
        # TRACKER
        args_tr
        ):
    ...
    for path, img, im0s, vid_cap in dataset:
            ...
            if new_ids_to_register or blob_res_list == [] or counter % args_tr.maxDisappeared == 0:
                ...
                if cen_blob_res_list.size == 0 or cen_det.size == 0:
                    ...
                    # if object detection empty nothing to update already updated
                        ...
                        # no obj det for blob dection, delete all blob detection
                        cen_blob_res_list, blob_res_list = [], []
                else:
                    ...
                    # if blob detections do not match loose those
                    # if in cand_ids blob missing -> no match with obj dec -> loose blob
                    index_to_delete = []
                    index_save_blob_anno = []
                    for i in range(0, len(cen_blob_res_list)):
                        if i not in cand_ids:
                            print("delete blob not matched", i)
                    ...
                    for i in range(0, len(cand_ids)):
                        if D[cand_ids[i], _filter_ids[i]] > args_tr.matching_threshold:
                            ...
                            # delete not matched blob
                            if cand_ids[i] not in index_to_delete:
        ...
        diff_time = ((datetime.now() - start_time).seconds + (
            (datetime.now() - start_time).microseconds) / 1000000) - diff_time_obj
...
if __name__ == "__main__":
    ...
    for img in run_blob_det_correct_centroid_tracker(opt, args):
        print("IMAGE")
