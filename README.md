...
* 2. [Description of the Use Case](#DescriptionoftheUseCase)
* 3. [Problem Statement](#ProblemStatement)
* 4. [Data Gathering](#DataGathering)
* 5. [Labeling](#Labeling)
	* 5.1. [Object detection labeling](#Objectdetectionlabeling)
	* 5.2. [Object tracking labeling](#Objecttrackinglabeling)
	* 5.3. [Dataset](#Dataset)
		* 5.3.1. [Additional data sources](#Additionaldatasources)
		* 5.3.2. [Mosaic dataset](#Mosaicdataset)
		* 5.3.3. [Synthetic dataset generation](#Syntheticdatasetgeneration)
	* 5.4. [Resulting datasets](#Resultingdatasets)
* 6. [Data Preprocessing](#DataPreprocessing)
	* 6.1. [Final Dataset](#FinalDataset)
	* 6.2. [Data Augmentation](#DataAugmentation)
* 7. [Model building](#Modelbuilding)
	* 7.1. [Background subtraction approaches](#Backgroundsubtractionapproaches)
	* 7.2. [Establish Baseline with background subtraction and blob detection](#EstablishBaselinewithbackgroundsubtractionandblobdetection)
	* 7.3. [Model selection](#Modelselection)
	* 7.4. [Evaluation Metric](#EvaluationMetric)
	* 7.5. [Training Enviornment](#TrainingEnviornment)
	* 7.6. [Background subtraction + Tiny YOLO](#BackgroundsubtractionTinyYOLO)
	* 7.7. [SSD (OLI)](#SSDOLI)
	* 7.8. [EfficientDet](#EfficientDet)
	* 7.9. [YOLO](#YOLO)
	* 7.10. [YOLOv4](#YOLOv4)
	* 7.11. [YOLOv5](#YOLOv5)
		* 7.11.1. [Why did we go with YOLOv5?](#WhydidwegowithYOLOv5)
* 8. [Model Training](#ModelTraining)
	* 8.1. [Freezing Layers](#FreezingLayers)
	* 8.2. [Adding Artificial Data](#AddingArtificialData)
	* 8.3. [Added Data Augmentation](#AddedDataAugmentation)
	* 8.4. [Hyperparameter Tuning](#HyperparameterTuning)
	* 8.5. [Final Results](#FinalResults)
* 9. [Deployment process](#Deploymentprocess)
...
##  2. <a name='DescriptionoftheUseCase'></a>Description of the Use Case
(Aleksandar Ilievski)  
...
##  3. <a name='ProblemStatement'></a>Problem Statement
...
##  4. <a name='DataGathering'></a>Data Gathering
(Aleksandar Ilievski)  
...
##  5. <a name='Labeling'></a>Labeling 
###  5.1. <a name='Objectdetectionlabeling'></a>Object detection labeling
(David Blumenthal)
...
###  5.2. <a name='Objecttrackinglabeling'></a>Object tracking labeling
(Maximilian Nitsche)
...
It turned out that this functionality is quite rare to never part of well known and common labeling tools. Therefore, we used the service [Amazon Sage Maker - Ground Truth](https://aws.amazon.com/de/sagemaker/groundtruth/), which included the free of charge labeling of 500 initial frames. We first had to provide the video frames to the service via an Amazon S3 storage bucket.
Unfortunately, the tool did not support a standard label format, such as YOLO or Pascal VOC or similar, so we had to convert existing labels into the appropriate JSON format by coding a short  script. However, Amazon Sage Maker Ground Truth allowed us to predict bounding boxes of the next frame based on the current ones. This active learning approach therefore simplified labeling significantly. An exemplary labeled frame can be found below.

![object tracking tool](./doku_resources/object_tracking_labeling.png)

###  5.3. <a name='Dataset'></a>Dataset 
(David Blumenthal)
...
####  5.3.1. <a name='Additionaldatasources'></a>Additional data sources 
(Maximilian Nitsche)
...
####  5.3.2. <a name='Mosaicdataset'></a>Mosaic dataset 
(Maximilian Nitsche)
...
####  5.3.3. <a name='Syntheticdatasetgeneration'></a>Synthetic dataset generation 
(Andrea Bartos)
...
###  5.4. <a name='Resultingdatasets'></a>Resulting datasets 
The resulting additional datasets are listed below. (Maximilian Nitsche)
...
##  6. <a name='DataPreprocessing'></a>Data Preprocessing
(David Blumenthal)
...
###  6.1. <a name='FinalDataset'></a>Final Dataset 

###  6.2. <a name='DataAugmentation'></a>Data Augmentation
(Andrea Bartos)
...
##  7. <a name='Modelbuilding'></a>Model building
###  7.1. <a name='Backgroundsubtractionapproaches'></a>Background subtraction approaches

###  7.2. <a name='EstablishBaselinewithbackgroundsubtractionandblobdetection'></a>Establish Baseline with background subtraction and blob detection
(Aleks)

###  7.3. <a name='Modelselection'></a>Model selection
(Andrea Bartos)
...
###  7.4. <a name='EvaluationMetric'></a>Evaluation Metric
 (Andrea Bartos)
...
###  7.5. <a name='TrainingEnviornment'></a>Training Enviornment
(David Blumenthal)
...
###  7.6. <a name='BackgroundsubtractionTinyYOLO'></a>Background subtraction + Tiny YOLO 
(Maximilian Nitsche)
...
###  7.7. <a name='SSDOLI'></a>SSD (OLI)

###  7.8. <a name='EfficientDet'></a>EfficientDet 
(Maximilian Nitsche)


###  7.9. <a name='YOLO'></a>YOLO
(David Blumenthal)
...
####  7.11.1. <a name='WhydidwegowithYOLOv5'></a>Why did we go with YOLOv5?
...
##  8. <a name='ModelTraining'></a>Model Training
...
###  8.1. <a name='FreezingLayers'></a>Freezing Layers 
(Andrea Bartos)
...
###  8.2. <a name='AddingArtificialData'></a>Adding Artificial Data 
(David Blumenthal)
...
###  8.3. <a name='AddedDataAugmentation'></a>Added Data Augmentation
(Andrea Bartos)
...
###  8.4. <a name='HyperparameterTuning'></a>Hyperparameter Tuning
(David Blumenthal)
...
###  8.5. <a name='FinalResults'></a>Final Results 
...
##  9. <a name='Deploymentprocess'></a>Deployment process
